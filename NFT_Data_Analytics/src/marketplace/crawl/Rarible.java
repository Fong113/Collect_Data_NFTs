...
public class Rarible extends Crawler {
	...
	protected void getData() throws CrawlTimeoutException, InternetConnectionException, Exception {
		String requestBody = "{\"size\":100 ,\"filter\":{\"verifiedOnly\":false,\"sort\":\"VOLUME_DESC\",\"blockchains\":[\""
				...
				+ MarketplaceType.RARIBLE.mapPeriodToPeriodParam.get(period)
				+ "\",\"hasCommunityMarketplace\":false,\"currency\":\"NATIVE\"}}";
		...
		for (JsonElement rowRaw : rowsRaw) {
			...
			row.add("floorPrice",
					isGet(rowRawObjStatistics, "floorPrice")
							? rowRawObjStatistics.getAsJsonObject("floorPrice").get("value")
							: null);
			row.add("floorPriceChange",
					isGet(rowRawObjStatistics, "floorPrice")
							? rowRawObjStatistics.getAsJsonObject("floorPrice").get("changePercent")
							: null);
			row.add("volume",
					isGet(rowRawObjStatistics, "amount") ? rowRawObjStatistics.getAsJsonObject("amount").get("value")
							: null);
			row.add("volumeChange",
					isGet(rowRawObjStatistics, "usdAmount")
							? rowRawObjStatistics.getAsJsonObject("usdAmount").get("changePercent")
							: null);
